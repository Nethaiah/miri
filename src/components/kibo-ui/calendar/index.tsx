...
import { getDay, getDaysInMonth, isSameDay, isWithinInterval, startOfDay, endOfDay } from "date-fns";
...
export type CalendarBodyProps = {
  ...
  onMoreClick?: (date: Date, features: Feature[]) => void;
};

export const CalendarBody = ({ features, children, onDayClick, onMoreClick }: CalendarBodyProps) => {
  ...
  const featuresByDay = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      result[day] = features.filter((feature) => {
        // Check if day is within feature range
        // Normalize times to start of day for comparison to include start/end days correctly
        const dayStart = startOfDay(currentDay);
        const featureStart = startOfDay(feature.startAt);
        const featureEnd = startOfDay(feature.endAt);
        
        return dayStart >= featureStart && dayStart <= featureEnd;
      });
    }
    return result;
    ...
    days.push(
      <div
        className={cn(
          ...
          onDayClick && "cursor-pointer hover:bg-muted/50 transition-colors"
        )}
        key={day}
        onClick={(e) => {
          if (onDayClick) {
            // Only trigger if clicking the cell background explicitly, 
            // OR relying on event bubbling if children handle stopPropagation
      ...
      >
        ...
        {featuresForDay.length > 3 && (
          <button
            className="block text-muted-foreground text-xs hover:text-foreground text-left w-full"
            onClick={(e) => {
              e.stopPropagation()
              onMoreClick?.(new Date(year, month, day), featuresForDay)
            }}
          >
...
export const CalendarItem = memo(
  ({ feature, className }: CalendarItemProps) => (
    <div 
      className={cn("flex items-center gap-2 rounded px-1 -mx-1", className)}
      style={{
        backgroundColor: `${feature.status.color}20`, // 20 = ~12% opacity hex
      }}
    >
      <div
      ...
      />
      <span className="truncate text-xs font-medium" style={{ color: feature.status.color === "#f59e0b" ? "inherit" : "inherit" }}>{feature.name}</span>
    </div>
  )
);
